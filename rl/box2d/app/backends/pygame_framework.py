...
from rl.box2d.app.framework import (FrameworkBase, Keys)
from rl.box2d.app.settings import fwSettings
from Box2D import (b2DrawExtended, b2Vec2)

try:
  from rl.box2d.app.backends.pygame_gui import (fwGUI, gui)
...
class PygameFramework(FrameworkBase):
  ...
  def run(self):
    ...
    while running:
      ...
      if self.settings.captureFrames:
        self.CaptureFrame()
  ...
  def _Keyboard_Event(self, key, down=True):
    ...
    if down:
      if key == Keys.K_z:  # Zoom in
        ...
        import IPython
        IPython.embed()
      elif key == Keys.K_F8:  # Toggle screen recording
        self.settings.captureFrames = not self.settings.captureFrames
  ...
  def CaptureFrame(self):
    ...
    bytes = surface.get_buffer().raw
    ary = np.frombuffer(bytes, dtype=np.uint8)
    ary = ary.reshape(surface.get_size() + (4,))
    ary = ary[:, :, :3]  # drop alpha channel

    if False:  # debugging
      import scipy.misc
      scipy.misc.imsave('/tmp/frame.png', ary)
    return ary
