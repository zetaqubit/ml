...
def main():
  ...
  def get_params_str():
    defaults = set(vars(parser.parse_args(['foo'])).items())
    args_set = set(vars(args).items())
    overrides = dict(args_set - defaults)
    option_strings = [a.option_strings for a in store_actions]
    long_to_short_names = {
      l[0][2:]: l[1][1:] for l in option_strings if len(l) > 1
    }
    params_str = ''
    for long_name in sorted(overrides.keys()):
      if long_name in ['env_name', 'exp_name']:
        continue
      name = long_to_short_names.get(long_name, long_name)
      value = overrides[long_name]
      value_str = str(value) if type(value) != bool else ''
      params_str += '_' if len(params_str) > 0 else ''
