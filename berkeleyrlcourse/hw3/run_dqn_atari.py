...
def atari_model(img_in, num_actions, scope, reuse=False):
  ...
  with tf.variable_scope(scope, reuse=reuse):
    ...
    with tf.variable_scope("convnet"):
      ...
      out = layers.convolution2d(out, num_outputs=32, kernel_size=8, stride=4,
                                 activation_fn=tf.nn.relu)
      out = layers.convolution2d(out, num_outputs=64, kernel_size=4, stride=2,
                                 activation_fn=tf.nn.relu)
      out = layers.convolution2d(out, num_outputs=64, kernel_size=3, stride=1,
                                 activation_fn=tf.nn.relu)
